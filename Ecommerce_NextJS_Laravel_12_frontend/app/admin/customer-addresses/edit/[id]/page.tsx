...
interface CustomerAddress {
  ...
  is_default: boolean;
  is_active: boolean;
  bangladeshi_area_id: number | null;
}
...
export default function EditCustomerAddressPage() {
  ...
  useEffect(() => {
    const fetchAddress = async () => {
      try {
        ...
        const data: { data?: CustomerAddress; message?: string } = await res.json();
        const addr = data.data || (data as any);
        setForm({
          ...
          bangladeshi_area_id: String(addr.bangladeshi_area_id || ""),
        });
      } catch (err) {
        console.error(err);
        alert("❌ Failed to load address");
  ...
  // Load areas when area filter changes
  useEffect(() => {
    if (!form || !form.area) {
      setAreas([]);
      return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      alert("❌ Failed to update customer address");
    } finally {
  ...
  if (!form) {
    return (
      <AdminLayout>
        <div className="p-4 text-center">Address not found</div>
      </AdminLayout>
    );
  }
    ...
    <AdminLayout>
      <div className="p-4 max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold">Edit Customer Address</h1>
          <Link
